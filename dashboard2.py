...
def build_bar_chart(summary_df: pd.DataFrame) -> alt.Chart:
    """Create a bar chart comparing total generation by energy type.

    The bar chart is sorted in descending order to direct the viewer’s
    attention to the largest contributors.  Colour coding is aligned
    with the rest of the dashboard for consistency.
    """
    palette = {
        "Other": "#6CA965",
        "Solar": "#E69F00",
    ...
    bar_chart = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("Total (TWh):Q", title="Total Generation (TWh)"),
            y=alt.Y("Energy:N", title="", sort="-x"),
            color=alt.Color("Energy:N", scale=alt.Scale(domain=list(palette.keys()), range=list(palette.values())), legend=None),
            tooltip=["Energy", alt.Tooltip("Total (TWh)", format=",.1f")],
        )
...
def main():
    ...
    bar_chart = build_bar_chart(summary_df)
    with col_left:
    ...
    # Create choropleth map; using a sequential colour scale that matches our palette
    if not geo_df.empty:
        st.markdown("### Geographical distribution (" + str(geo_year) + ")")
        fig = px.choropleth(
            geo_df,
            locations="Code",
            color="Total",
            ...
            color_continuous_scale=["#fef6e4", "#6CA965", "#4E79A7"],
            labels={"Total": "Total generation (TWh)"},
        )
        fig.update_geos(showcoastlines=False, showland=True, landcolor="#f7f7f7", showcountries=True, countrycolor="white")
        fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), coloraxis_colorbar=dict(title="TWh"))
