...
# Custom color palette applied consistently to all charts
...
def main():
    ...
    with col_a1:
        st.plotly_chart(
            px.line(partner_sales,
                    x='Month',
                    y=['Units Sold', 'Units Sold LY'],
    ...
    with col_b1:
        # Display the revenue curve here (swapped position)
    ...
    with col_b2:
        # Keep the error distribution chart
        st.plotly_chart(
            px.histogram(error_df,
                         x='Error (units)',
                         nbins=30,
