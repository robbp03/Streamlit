...
def main():
    ...
    # Annotate milestone
    # Use native datetime objects for vertical annotations to avoid Timestamp
    # addition errors in Plotly (see pandas deprecation).  Using Python
    # datetime ensures `add_vline` works correctly without arithmetic on
    # pandas.Timestamp objects.
    import datetime as _dt
    # Plotly's add_vline can raise errors when mixing datetime objects with numeric
    # values internally.  Instead, add vertical lines via add_shape with
    # explicit coordinates.  These lines span the full y-axis using yref='paper'.
    fig_global.add_shape(
        type="line",
        x0=_dt.datetime(2021, 3, 31), x1=_dt.datetime(2021, 3, 31),
        y0=0, y1=1, xref="x", yref="paper",
        line=dict(dash="dot"),
    )
    fig_global.add_annotation(
        x=_dt.datetime(2021, 3, 31), y=1.02, xref="x", yref="paper", text="Early ramp‑up",
        showarrow=False, align="left", xanchor="left"
    )
    fig_global.add_shape(
        type="line",
        x0=_dt.datetime(2022, 1, 31), x1=_dt.datetime(2022, 1, 31),
        y0=0, y1=1, xref="x", yref="paper",
        line=dict(dash="dot"),
    )
    fig_global.add_annotation(
        x=_dt.datetime(2022, 1, 31), y=1.02, xref="x", yref="paper", text="Omicron wave",
        showarrow=False, align="left", xanchor="left"
    )
    ...
    # Create heatmap only if data is available; otherwise inform the user.
    if heat_filtered.size == 0 or heat_filtered.dropna(how="all").empty:
        st.write("No data available for the selected date range and regions for the heatmap.")
    else:
        fig_heat, ax_heat = plt.subplots(figsize=(10, 5))
        sns.heatmap(
            heat_filtered,
            cmap="YlGnBu",
            cbar_kws={"label": "% with ≥1 dose"},
            ax=ax_heat
        )
